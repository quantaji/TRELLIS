...
if [ "$BASIC" = true ]; then
    pip install pillow imageio imageio-ffmpeg tqdm easydict opencv-python-headless scipy ninja rembg onnxruntime trimesh open3d xatlas pyvista pymeshfix igraph transformers pandas objaverse huggingface_hub open_clip_torch black ipykernel
...
if [ "$VOX2SEQ" = true ]; then
    if [ "$PLATFORM" = "cuda" ]; then
        ...
        pip install --no-build-isolation /tmp/extensions/vox2seq
    else
