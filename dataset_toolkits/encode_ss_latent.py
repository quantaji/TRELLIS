...
def get_voxels(instance):
    ...
    if os.path.exists(os.path.join(opt.output_dir, "voxels", f"{instance}_overall_centered.ply")):
        position = utils3d.io.read_ply(os.path.join(opt.output_dir, "voxels", f"{instance}_overall_centered.ply"))[0]
    coords = ((torch.tensor(position) + 0.5) * opt.resolution - 0.5).round().int().contiguous()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enc_pretrained", type=str, default="microsoft/TRELLIS-image-large/ckpts/ss_enc_conv3d_16l8_fp16", help="Pretrained encoder model")
    parser.add_argument("--model_root", type=str, default="results", help="Root directory of models")
    parser.add_argument("--enc_model", type=str, default=None, help="Encoder model. if specified, use this model instead of pretrained model")
