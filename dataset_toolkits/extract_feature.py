        ...
        with ThreadPoolExecutor(max_workers=8) as loader_executor, ThreadPoolExecutor(max_workers=8) as saver_executor:
            ...
            def loader(sha256):
                try:
                    ...
                    indices = np.round((positions + 0.5) * 64 - 0.5).astype(np.long)
                    if os.path.exists(os.path.join(opt.output_dir, "voxels", f"{sha256}_overall_centered.ply")):
                        _overall_positions = utils3d.io.read_ply(os.path.join(opt.output_dir, "voxels", f"{sha256}_overall_centered.ply"))[0]
                        indices = np.round((_overall_positions + 0.5) * 64 - 0.5).astype(np.long)
                    load_queue.put((sha256, data, positions, indices))
            ...
            for _ in tqdm(range(len(sha256s)), desc="Extracting features"):
                ...
                sha256, data, positions, indices = load_queue.get()
                positions = torch.from_numpy(positions).float().cuda()
                indices = torch.from_numpy(indices).long().cuda()
                assert torch.all(indices >= 0) and torch.all(indices < 64), f"Some vertices are out of bounds {indices.max()} {indices.min()}"
                n_views = len(data)
                N = positions.shape[0]
                pack = {
